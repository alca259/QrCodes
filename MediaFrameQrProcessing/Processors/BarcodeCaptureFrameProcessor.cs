namespace MediaFrameQrProcessing.Processors
{
    ...
    {
        public BarcodeCaptureFrameProcessor(MediaFrameSourceFinder mediaFrameSourceFinder, DeviceInformation videoDeviceInformation, string mediaEncodingSubtype, MediaCaptureMemoryPreference memoryPreference = MediaCaptureMemoryPreference.Cpu)
        ...
        protected override bool ProcessFrame(MediaFrameReference frameReference)
        {
            ...
            using (var bitmap = frameReference.VideoMediaFrame.SoftwareBitmap)
            {
                try
                {
                    ...
                    var zxingResult = ZXingBarcodeDecoder.DecodeBufferToCode(buffer, bitmap.PixelWidth, bitmap.PixelHeight, BitmapFormat.BGR32);
