namespace MediaFrameQrProcessing.Processors
{
    ...
    {
        protected Action<VideoDeviceController> videoDeviceControllerInitialiser;
        protected string mediaEncodingSubtype;
        protected MediaFrameSourceFinder mediaFrameSourceFinder;
        protected DeviceInformation videoDeviceInformation;
        protected MediaCaptureMemoryPreference memoryPreference;
        protected MediaCapture mediaCapture;
        protected bool isFinished = false;

        public MediaCaptureFrameProcessor(MediaFrameSourceFinder mediaFrameSourceFinder, DeviceInformation videoDeviceInformation, string mediaEncodingSubtype, MediaCaptureMemoryPreference memoryPreference = MediaCaptureMemoryPreference.Cpu)
        {
        ...
        public async Task ProcessFramesAsync(TimeSpan? timeout, Action<T> resultCallback = null)
        {
                ...
                using (var frameReader = await this.mediaCapture.CreateFrameReaderAsync(mediaFrameSource, this.mediaEncodingSubtype))
                {
                    ...
                    while (!isFinished)
                    {
                        ...
                        if (timeout.HasValue)
                        {
                            ...
                            isFinished = (this.Result != null) || (timedOut);
                        }
                    }
