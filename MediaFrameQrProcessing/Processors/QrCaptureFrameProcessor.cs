namespace MediaFrameQrProcessing.Processors
{
    ...
    {
        public QrCaptureFrameProcessor(MediaFrameSourceFinder mediaFrameSourceFinder, DeviceInformation videoDeviceInformation, string mediaEncodingSubtype, MediaCaptureMemoryPreference memoryPreference = MediaCaptureMemoryPreference.Cpu)
        ...
        protected override bool ProcessFrame(MediaFrameReference frameReference)
        {
            ...
            using (var bitmap = frameReference.VideoMediaFrame.SoftwareBitmap)
            {
                try
                {
                    ...
                    var zxingResult = ZXingQRCodeDecoder.DecodeBufferToQRCode(buffer, bitmap.PixelWidth, bitmap.PixelHeight, BitmapFormat.BGR32);
